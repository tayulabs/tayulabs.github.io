(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .tayu-sector-device{border:1px solid var(--border);background:var(--panel2);border-radius:18px;padding:15px}
      .tayu-sector-device-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap}
      .tayu-sector-device-head h4{margin:0;font-size:17px}.tayu-sector-device-key{font-size:11px;color:var(--muted);margin-top:4px}
      .tayu-sector-device-meta{display:flex;gap:7px;flex-wrap:wrap;margin-top:9px}
      .tayu-sector-pill{padding:5px 8px;border-radius:999px;border:1px solid var(--border);background:var(--panel);font-size:10px;font-weight:850}
      .tayu-sector-status{padding:6px 9px;border-radius:999px;font-size:10px;font-weight:900;background:rgba(239,68,68,.10);color:var(--danger)}
      .tayu-sector-status.online{background:rgba(91,193,47,.12);color:var(--brand)}
      .tayu-sector-resources{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:13px}
      .tayu-sector-resource{border:1px solid var(--border);background:var(--panel);border-radius:15px;padding:12px}
      .tayu-sector-resource-top{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}.tayu-sector-resource h5{margin:0;font-size:14px}
      .tayu-sector-resource small{color:var(--muted)}.tayu-sector-state{font-size:17px;font-weight:900;margin-top:8px}
  ...
  function latestTelemetryMap() {
    ...
    rows.forEach(row => {
      ...
      if (!previous || new Date(row.time || 0) >= new Date(previous.time || 0)) map.set(String(key), row);
    });
    return map;
  }
  ...
  function deviceOnline(device, telemetry) {
  ...
  function hideLegacyNovaCamaroneras() {
    const view = document.getElementById('camaroneras');
    if (!view) return;
    view.querySelectorAll('.nova-edge-panel').forEach(element => {
      if (!element.dataset.tayuSectorOwned) element.style.display = 'none';
    });
    view.querySelectorAll('details').forEach(details => {
      const summary = details.querySelector(':scope > summary')?.textContent || '';
      if (/NOVA\s*EDGE/i.test(summary)) details.style.display = 'none';
    });
  }
  ...
  async function loadResources(deviceKey, force = false) {
    ...
    if (!force && resourceCache.has(key)) return resourceCache.get(key);
  ...
  function renderOutput(resource, device, telemetry, sector) {
    const payload = payloadObject(telemetry);
    const key = String(resource.resource_key || '');
    const state = readOutput(payload, key);
    const on = state === true;
    const assignment = resource.assignment || null;
    const enabled = assignment?.enabled !== false;
    const mode = String(assignment?.settings?.mode || 'manual').toLowerCase();
    const manual = mode === 'manual' || !mode;
    const online = deviceOnline(device, telemetry);
    const controllable = resource.controllable !== false && enabled && manual;
    const stateText = state === undefined ? '— SIN LECTURA' : on ? '● ENCENDIDO' : '○ APAGADO';
    const modeText = manual ? 'Manual' : mode === 'automatic' ? 'Automático' : mode === 'timer' ? 'Timer' : mode;
    ...
    return `<article class="tayu-sector-resource" data-sector-output="${esc(key)}" data-device-key="${esc(device.device_key)}" data-sector="${esc(sector)}">
      ...
      ${assignment ? '' : '<div class="tayu-sector-note">Sin asignación específica. Puedes configurarlo desde Dispositivos → Configurar IoT.</div>'}
      ${!enabled ? '<div class="tayu-sector-note">Este recurso está desactivado.</div>' : ''}
      <div class="tayu-sector-control">
        <button type="button" class="btn ${on ? 'ghost' : ''}" data-sector-toggle ${(!online || !controllable || state === undefined) ? 'disabled' : ''} data-next="${on ? '0' : '1'}">
          ${!online ? 'DISPOSITIVO OFFLINE' : !enabled ? 'RECURSO INACTIVO' : !manual ? `CONTROL ${esc(modeText.toUpperCase())}` : state === undefined ? 'ESPERANDO ESTADO' : on ? 'APAGAR' : 'ENCENDER'}
        </button>
      </div>
    </article>`;
  }
  ...
  function renderDevice(device, resourceData, telemetry, sector) {
    const resources = Array.isArray(resourceData?.resources) ? resourceData.resources : [];
    const outputs = resources.filter(resource => resource.resource_type === 'digital_output');
    const online = deviceOnline(device, telemetry);
    const profile = device.profile_name || device.profile_key || device.device_type || 'Dispositivo IoT';
    const site = device.site_name || resourceData?.device?.site_name || 'Sin sitio';

    return `<section class="tayu-sector-device">
      <div class="tayu-sector-device-head">
        <div>
          <h4>${esc(device.name || device.device_key)}</h4>
          <div class="tayu-sector-device-key">${esc(device.device_key)}</div>
          <div class="tayu-sector-device-meta">
            <span class="tayu-sector-pill">${esc(profile)}</span>
            <span class="tayu-sector-pill">Sitio: ${esc(site)}</span>
            <span class="tayu-sector-pill">Sector: ${esc(LABELS[sector] || sector)}</span>
          </div>
        </div>
        <span class="tayu-sector-status ${online ? 'online' : ''}">${online ? 'ONLINE' : 'OFFLINE'}</span>
      </div>
      ${outputs.length
  ...
  async function renderSector(sector, force = false) {
    ...
    try {
      ...
      const devices = (Array.isArray(window.__tayuRealDevices) ? window.__tayuRealDevices : [])
        .filter(device => String(device?.site_sector || '').toLowerCase() === sector);
      ...
      <div class="tayu-sector-iot-list"><div class="tayu-sector-empty">Cargando dispositivos IoT…</div></div>`;

      const listHost = host.querySelector('.tayu-sector-iot-list');
      if (!devices.length) {
        listHost.innerHTML = `<div class="tayu-sector-empty">No hay dispositivos IoT asignados a ${esc(LABELS[sector])}. La asignación se administra desde Cloud Admin.</div>`;
        return;
      }
      ...
      const rows = await Promise.all(devices.map(async device => {
        try {
          const resources = await loadResources(device.device_key, force);
          return renderDevice(device, resources, telemetryMap.get(String(device.device_key)), sector);
        } catch (error) {
          return `<section class="tayu-sector-device"><div class="tayu-sector-device-head"><div><h4>${esc(device.name || device.device_key)}</h4><div class="tayu-sector-device-key">${esc(device.device_key)}</div></div></div><div class="tayu-sector-note">No se pudieron cargar los recursos: ${esc(error.message)}</div></section>`;
        }
      }));
  ...
  async function refreshSector(sector) {
    ...
    resourceCache.clear();
    await renderSector(sector, true);
  }

  async function toggleOutput(button) {
    const card = button.closest('[data-sector-output]');
    if (!card || typeof window.setGenericOutput !== 'function') return;
    const deviceKey = card.dataset.deviceKey;
    const outputKey = card.dataset.sectorOutput;
    const sector = card.dataset.sector;
    const next = button.dataset.next === '1';
    try {
      await window.setGenericOutput(deviceKey, outputKey, next, button);
      await refreshSector(sector);
    } catch (error) {
      console.error('Sector IoT output:', error);
    }
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (refresh) {
      ...
      refreshSector(refresh.dataset.sectorIotRefresh);
      return;
    }
