(() => {
  ...
  function palette(type,percent){
    if(percent<=33)return {level:'low',accent:'#EF4444',accent2:'#B91C1C'};
    if(percent<=66)return {level:'medium',accent:'#F59E0B',accent2:'#D97706'};
  ...
  function paint(host,type,cfg,d,v){
    ...
    const colors=palette(type,percent);
    ...
    const percentLabel=host.querySelector('[data-av-percent]');
    if(percentLabel)percentLabel.style.color=colors.accent;
    ...
    if(gauge){
      ...
      gauge.setAttribute('stroke-dasharray',`${percent.toFixed(2)} 100`);
    }

    if(type==='kpi'){
      const h=history(d,v);spark(host.querySelector('[data-av-spark]'),h);
      const polyline=host.querySelector('[data-av-spark] polyline');
      if(polyline)polyline.style.stroke=colors.accent;
      const current=host.querySelector('[data-av-value]');
      if(current)current.style.color=colors.accent;
  ...
  function setPreviewMode(custom){
    const section=$('modbus');if(section)section.classList.toggle('tayu-v4-custom-preview',custom);
    const canvas=$('mbPreviewChart');if(canvas)canvas.style.display=custom?'none':'';
    if(!custom)canvas?.parentElement?.querySelector('.tayu-av-host[data-av-v4-preview]')?.remove();
  }
  ...
  async function renderPreview(loadHistory=false){
    ...
    let host=box.querySelector('.tayu-av-host[data-av-v4-preview]');
    if(!host){
      box.querySelector('.tayu-av-host[data-av-v3-preview],.tayu-av-host[data-av-v2-preview],.tayu-av-host[data-av-preview]')?.remove();
      host=document.createElement('div');host.className='tayu-av-host';host.dataset.avV4Preview='1';box.appendChild(host);
    }
  ...
  function renderDashboardCustom(){
    ...
    document.querySelectorAll('#dashboard .dashboard-widget[data-widget-id]').forEach(widget=>{
      ...
      if(!box)return;
      const canvas=box.querySelector('canvas.dashboard-modbus-chart,canvas.tayu-av-v4-canvas,canvas.tayu-av-v3-canvas,canvas.tayu-av-v2-canvas,canvas.tayu-av-base-canvas');
      if(!custom){
        box.querySelector('.tayu-av-host[data-av-v4-dashboard]')?.remove();
        if(canvas?.classList.contains('tayu-av-v4-canvas')){
          canvas.classList.remove('tayu-av-v4-canvas');canvas.classList.add('dashboard-modbus-chart');canvas.style.display='';
        }
        return;
      }
      if(canvas){
        if(canvas._chart){try{canvas._chart.destroy();}catch(_){}canvas._chart=null;}
        canvas.classList.remove('dashboard-modbus-chart','tayu-av-v3-canvas','tayu-av-v2-canvas','tayu-av-base-canvas');
        canvas.classList.add('tayu-av-v4-canvas');canvas.style.display='none';
      }
      let host=box.querySelector('.tayu-av-host[data-av-v4-dashboard]');
      if(!host){
        box.querySelector('.tayu-av-host[data-av-v3-dashboard],.tayu-av-host[data-av-v2-dashboard],.tayu-av-host[data-av-dashboard]')?.remove();
        host=document.createElement('div');host.className='tayu-av-host';host.dataset.avV4Dashboard='1';box.appendChild(host);
      }
  ...
  function installSafePreview(){
    if(window.updateMbPreview?.__tayuVisualV4)return;
    ...
    safe.__tayuVisualV4=true;window.updateMbPreview=safe;
  }

  function installSafeAdd(){
    if(window.addModbusChartWidget?.__tayuVisualV4)return;
    ...
    const safe=function(){
      ...
      try{
        ...
        if(!widgetId){
          widgetId=window.newWidgetInstance?.('modbusChart');
          if(!widgetId)throw new Error('No se pudo crear el widget.');
          window.dashboardWidgetIds=[...ids(),widgetId];
        }
        ...
        console.error('Guardar visualización IoT:',error);
        alert(`No se pudo guardar la gráfica: ${error?.message||error}`);
      }
    };
    safe.__tayuVisualV4=true;window.addModbusChartWidget=safe;
  }

  function wrapDashboardRender(){
    if(window.renderDashboardModbusChart?.__tayuVisualV4)return;
    ...
    wrapped.__tayuVisualV4=true;window.renderDashboardModbusChart=wrapped;
  }

  function wrapWidgetRender(){
    if(window.renderDashboardWidgets?.__tayuVisualV4)return;
    ...
    wrapped.__tayuVisualV4=true;window.renderDashboardWidgets=wrapped;
  }
