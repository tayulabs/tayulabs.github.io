(() => {
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .tayu-av-hidden-field{display:none!important}.tayu-av-v2-canvas{display:none!important}
  ...
  function syncFields(){
    ...
    if(builder&&!builder.querySelector('.tayu-av-help')){
      ...
      note.innerHTML='<b>Visualizaciones IoT:</b> Tanque y Liquid fill muestran el nivel animado; los gauges muestran el valor actual dentro del rango Mín/Máx; KPI conserva el histórico para su mini tendencia.';
  ...
  function boot(){
    ...
    document.addEventListener('click',event=>{if(event.target?.closest?.('.nav button[data-view="modbus"]'))setTimeout(syncFields,120);},true);
    window.addEventListener('pageshow',()=>setTimeout(syncFields,120));
  }
